...
const envSchema = z.object({
  ...
  USE_BULLMQ: z.string().default('true').transform((val: string) => val === 'true'),

  SESSION_SECRET: z.string().min(32, { message: 'SESSION_SECRET must be at least 32 characters' }),
  GOOGLE_OAUTH_REDIRECT_URI: z.string().url({ message: 'GOOGLE_OAUTH_REDIRECT_URI must be a valid URL' }),
  FRONTEND_PUBLIC_URL: z.string().url({ message: 'FRONTEND_PUBLIC_URL must be a valid URL' }),
  COOKIE_DOMAIN: z.string().default(''),
  COOKIE_SECURE: z.string().default('true').transform((val: string) => val === 'true'),
  COOKIE_SAMESITE: z.enum(['lax', 'strict', 'none']).default('lax'),
  COOKIE_MAX_AGE_MS: z.coerce.number().int().positive().default(60 * 60 * 24 * 7 * 1000),
